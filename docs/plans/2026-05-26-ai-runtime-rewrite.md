...
- **Live model fetching + context meter + auto-defaults shipped** (commit 387fb29):
  - `GET /admin/api/ai/providers/:id/models` now enriches Anthropic/OpenAI models with prices and `contextWindow` from the live OpenRouter catalogue (`server/ai/pricing/`, in-memory 6h cache over `ai_model_pricing`). OpenRouter and Ollama self-populate.
  - `<ContextMeter>` in the composer shows "context used / window". Window sourced from the enriched model; used sourced from `agentContextTokens` (the provider-normalised input from the latest `usage` event, persisted to `ai_conversations.context_tokens` for reload). Normalisation in `server/ai/contextTokens.ts`.
  - Credential creation auto-seeds empty scopes with the new credential and its smartest-tier model (`seedEmptyDefaults` in `credentials.ts`). Best-effort — never fails the create.
  - DB migrations 013 (`ai_model_pricing`), 014 (`context_tokens` column on `ai_conversations`), 015 (`context_window` column on `ai_model_pricing`).
